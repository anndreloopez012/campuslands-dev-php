# 15. Autenticacion local - diseno 3D arquitectura
...
Trabajas en un modulo PHP mantenible para una plataforma relacionada con diseno 3D arquitectura. La prioridad es estructura, seguridad, persistencia y respuestas predecibles. El caso numero 15 se enfoca en autenticacion local.
...
Practicar validar acceso con sesiones y roles usando PHP y aplicando estos conceptos: **password_hash, password_verify, roles**.
...
```bash
docker compose run --rm php php avanzado/ejercicio-015-autenticacion-local-diseno-3d-arquitectura/resoluciones/tu-nombre/ejercicio.php
```

Ejemplo con PHP instalado localmente:

```bash
php avanzado/ejercicio-015-autenticacion-local-diseno-3d-arquitectura/resoluciones/tu-nombre/ejercicio.php
```
