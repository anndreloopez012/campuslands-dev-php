# 06. Concurrencia local - kickboxing
...
Actuas como responsable tecnico de un sistema PHP relacionado con kickboxing. Debes pensar en escalabilidad local, errores, auditoria, integraciones y mantenimiento. El caso numero 6 se enfoca en concurrencia local.
...
Practicar prevenir inconsistencias simples usando PHP y aplicando estos conceptos: **locks, transacciones, idempotencia**.
...
```bash
docker compose run --rm php php experto/ejercicio-006-concurrencia-local-kickboxing/resoluciones/tu-nombre/ejercicio.php
```

Ejemplo con PHP instalado localmente:

```bash
php experto/ejercicio-006-concurrencia-local-kickboxing/resoluciones/tu-nombre/ejercicio.php
```
