# 37. Configuracion por entorno - accion y aventura
...
Actuas como responsable tecnico de un sistema PHP relacionado con accion y aventura. Debes pensar en escalabilidad local, errores, auditoria, integraciones y mantenimiento. El caso numero 37 se enfoca en configuracion por entorno.
...
Practicar separar valores locales y sensibles usando PHP y aplicando estos conceptos: **env, config, defaults**.
...
```bash
docker compose run --rm php php experto/ejercicio-037-configuracion-por-entorno-accion-y-aventura/resoluciones/tu-nombre/ejercicio.php
```

Ejemplo con PHP instalado localmente:

```bash
php experto/ejercicio-037-configuracion-por-entorno-accion-y-aventura/resoluciones/tu-nombre/ejercicio.php
```
...
- La salida responde al contexto de accion y aventura.
...
echo "Ejercicio PHP - accion y aventura\n";
```
